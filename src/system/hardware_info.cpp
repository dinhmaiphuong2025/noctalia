...
namespace {
  ...
  std::string readCpuModel() {
    ...
    std::unordered_map<std::string, int> partCounts;
    while (std::getline(file, line)) {
      ...
      if (line.starts_with("CPU part")) {
        const auto colonPos = line.find(':');
        if (colonPos != std::string::npos) {
          ++partCounts[StringUtils::trim(line.substr(colonPos + 1))];
        }
      }
    }

    // big.LITTLE/prime SoCs report one "CPU part" per cluster. Pick the part with
    // the most cores (usually the efficiency cluster) to identify the SoC.
    std::string part;
    int partCount = 0;
    for (const auto& [candidate, count] : partCounts) {
      if (count > partCount) {
        partCount = count;
        part = candidate;
      }
